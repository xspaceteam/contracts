...
contract Staking is Ownable, ReentrancyGuard {
    ...
    modifier noContract() {
        require(tx.origin == msg.sender, "StakingXSP: Contract not allowed to interact");
        _;
    }
    ...
    function redeem(uint256 _redeemAmount) external nonReentrant noContract {
        require(enabled, "Staking is not enabled");
        StakeDetail storage stakeDetail = stakers[msg.sender];
        require(stakeDetail.firstStakeAt > 0, "StakingXSP: no stake");
        require(
        ...
        require(
            ...
            "StakingXSP: redeem amount must be less than principal"
        );
        ...
        require(
            ...
            "StakingXSP: transfer failed"
        );
        emit Redeem(msg.sender, _redeemAmount.add(claimAmount));

        sXSP.transferFrom(_msgSender(), address(this), _redeemAmount);
        sXSP.burn(address(this), _redeemAmount);
        xspStake = xspStake.sub(_redeemAmount);
        xspDistribute = xspDistribute.add(claimAmount);
    }
